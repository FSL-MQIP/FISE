...
# (i) Sample the temperature distribution # Sarah figure out how to implement Laplace distribution
temps <- vector()
for (i in 1:n_sim){
...
for (i in 1:(n_sim * n_units * n_day)) {
  allele_index <-
    which(spore_growth_import$rpoBAT == data$STorAT[i]) #Find row in growth parameter data that corresponds to allele sample
  row <- data[i,]
