...
#Size is for n_sim bulk tanks, n_half_gal half gallon lots, n_day days (14-35)
...
#Repeat   (Repeat 14..35 each number 10 times) 100 times
day <- rep(rep(seq(14, 35), each = n_halfgal), times = n_sim)
...
data$MPN_init_mL <- MPN_init_mL
# data$MPN_init_mL <- MPN_init_mL - 0.3066 #Subtract the reduction in CFU for towel intervention
# data$MPN_init_mL <- MPN_init_mL - 0.0183 #Subtract the reduction in CFU for training intervention

data$MPN_init_mL[data$MPN_init_mL<0 ] <- 0
data$MPN_init_mL[data$MPN_init_mL == 0] <- 0.01

data$logMPN_init <- log10(data$MPN_init_mL) #Original version
...
save(data_original, file = "data_original.RData")
# save(data_towel, file = "data_towel.RData")
# save(data_training, file = "data_training.RData")

# load("data_original.RData")
# load("data_towel.RData")
