...
Just look at MCsim_013020.R for more info. 
...
compare_means %>% spread(type, mean) %>% mutate( Reduction = `Initial MPN` - `Pot Intervention`)-> means.wide
...
```{r}
#want mean and sd by day in shelf life for each of the 3 versions
summary_original <- data_original%>%
  group_by(day)%>%
  summarise(mean=mean(count),
            sd=sd(count))

summary_towel <- data_towel%>%
  group_by(day)%>%
  summarise(mean=mean(count),
            sd=sd(count))
...
```{r}
d21o <- data_original[data_original$day=="21",]
d35o <- data_original[data_original$day=="35",]

d21t <- data_training[data_training$day=="21",]
d35t <- data_training[data_training$day=="35",]

d21to <- data_towel[data_towel$day=="21",]
d35to <- data_towel[data_towel$day=="35",]
```

```{r}
#number of samples greater than 20,000 CFU/ml (aka 4.23)
```


```{r}
hist(d21o$count,xlim=c(0,9),ylim=c(0,5000),breaks=40)
hist(d21t$count,breaks=40,ylim=c(0,5000),xlim=c(0,9))
hist(d21to$count,breaks=40,ylim=c(0,5000),xlim=c(0,9))
#overlay these three plots different colors

hist(d35o$count,breaks=8,ylim=c(0,5000),xlim=c(0,9))
hist(d35t$count,breaks=20,ylim=c(0,5000),xlim=c(0,9))
hist(d35to$count,breaks=20,ylim=c(0,5000),xlim=c(0,9))
```
